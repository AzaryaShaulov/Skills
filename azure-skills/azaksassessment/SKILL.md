...
Drives the bundled `AzAKSAssessment` PowerShell toolchain (eight `*.ps1` scripts + `Queries.kql` + `run.ps1` orchestrator, all under [`./artifacts/`](./artifacts/)) in a clean, reproducible, **strictly read-only** way and produces one HTML report per AKS-bearing subscription plus an `index.html` summary.
...
All scripts live in [`./artifacts/`](./artifacts/). `cd` into that directory (or invoke with full paths) before running.
...
> If you invoke individual phase scripts directly (not via `run.ps1`), they default `-OutputDir` / `-DataDir` to `<artifacts>/data` and `<artifacts>/reports`. Pass `-OutputDir` / `-DataDir` explicitly to opt in to the dated layout outside of `run.ps1`.
